...
async function sendEmail(option: MailOption) {
...
function mailTemplate(content: string, buttonURL: string, buttonText: string): string {
  return `
  <!DOCTYPE html>
  <html>
...
export async function sendPasswordResetEmail(id: string, email: string, plainToken: string): Promise<void> {

  const baseLink = process.env.FRONTEND_URL + FRONTEND_ROUTES.AUTH.RESET_PASSWORD;

  const body = mailTemplate(
    "We have received a request to reset your password. Please reset your password using the link below.",
    `${baseLink}?userId=${id}&token=${plainToken}`,
    "Reset Password"
  );
